...
| `dir` | `'ltr' \| 'rtl'` | — | Text direction. Rendered on `.app-wrapper` and written to `<html dir>` while the layout is mounted (the previous value is restored on unmount). Pair it with the RTL stylesheet — see [`useDirection`](/composables/component#usedirection). |
