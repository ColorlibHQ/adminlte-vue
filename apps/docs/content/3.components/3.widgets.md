...
> **`body` and `footer` are raw HTML.** They are written with `v-html` to match AdminLTE's markup, so
> they must only ever contain trusted, developer-authored strings. Passing user-generated content
> (comments, notifications, anything from an API you don't control) straight into these fields is a
> stored-XSS vector. Either sanitize it first, or use the `body` / `footer` slots — slot content goes
> through Vue's normal template escaping.
...
| `body` | `string` | Body markup, inserted as HTML — **trusted content only**. |
| `footer` | `string` | Footer markup, inserted as HTML — **trusted content only**. |
...
| Name | Description |
| --- | --- |
| `body` | Escaped replacement for `item.body`. Slot props: `item`, `index`. |
| `footer` | Escaped replacement for `item.footer`. Slot props: `item`, `index`. |
