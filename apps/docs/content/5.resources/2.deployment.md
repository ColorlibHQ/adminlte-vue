...
section: Resources
order: 2
description: Building the library and deploying a Nuxt app that uses it.
---
...
3. A `closeBundle` hook copies core's prebuilt stylesheets into `dist/css/`: `adminlte.css` and `adminlte.rtl.css` (the `./css` and `./css/rtl` exports), the two opt-in palettes `adminlte-colors.css` and `adminlte-colors-v3.css` (`./css/colors` and `./css/colors/v3` — see [Color mode](/guide/color-mode)), and `adminlte-docs.css` (`./css/docs`). A stylesheet that cannot be read fails the build rather than shipping an export that resolves to nothing.
