...
section: Guide
order: 2
description: Light/dark/auto color mode with an SSR-safe, flash-free theme, plus the opt-in color palettes.
---
...
Color mode is about light vs. dark. If you also want *more colors*, AdminLTE core ships two
**opt-in** stylesheets, re-exported by this package. Neither is loaded by default — `@adminlte/vue/css`
is unchanged — and you load **one or the other**, never both, because they share class names.

| Export | Contents |
| --- | --- |
| `@adminlte/vue/css/colors` | The designed palette (core 4.4.0) — 14 extra colors (orange, amber, olive, teal, sky, indigo, violet, fuchsia, pink, navy, steel, slate, graphite, midnight), each generated so white text on it clears WCAG AA. |
| `@adminlte/vue/css/colors/v3` | The AdminLTE 3 palette (core 4.5.0) — the original 18 colors with their v3 values and v3's text-color rule. |

Both add the same class families for their colors: `.bg-*`, `.text-bg-*`, `.text-*`, `.border-*`,
`.link-*`, `.bg-gradient-*`, `.card-*`, `.callout-*` and `.direct-chat-*`.

```ts [nuxt.config.ts]
export default defineNuxtConfig({
  modules: ['@adminlte/nuxt'],
  css: ['@adminlte/vue/css/colors'], // or '@adminlte/vue/css/colors/v3'
})
```

```ts [main.ts — plain Vue]
import '@adminlte/vue/css'
import '@adminlte/vue/css/colors'
```

### Making a palette color the primary

With one of the sheets loaded, `data-lte-primary` on `<html>` (or on any container) repoints
Bootstrap's `primary` at that color — buttons, links, pagination, `.nav-pills`, progress bars, list
groups, accordions and form-control / checkbox focus rings all follow, in both color modes:

```ts [nuxt.config.ts]
export default defineNuxtConfig({
  app: { head: { htmlAttrs: { 'data-lte-primary': 'teal' } } },
})
```

That means the components you already render with `theme="primary"` — `LteCard`, `LteSmallBox`,
`LteButton`, `LteCallout`, `LteDirectChat`, … — become teal without touching your markup. The typed
`theme` props stay the eight Bootstrap names; the palette colors are used as plain classes
(`class="text-bg-teal"`, `class="callout callout-olive"`) wherever you write markup yourself.

### AA text on the v3 palette

Eight of the AdminLTE 3 colors fall short of 4.5:1 with the white text v3 gave them. Adding
`data-lte-contrast="aa"` alongside the v3 sheet flips exactly those eight to the ink that passes,
leaving every background unchanged. It is a no-op for the designed palette, which is AA by
construction.
