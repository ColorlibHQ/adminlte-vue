...
| `navigate` | `(href: string) => void` | — | Callback invoked when a command-palette result is chosen **or** a sidebar link is clicked. Wire it to your router's programmatic navigation. |
...
### `linkComponent` or `navigate`?

Either one gives you client-side navigation in the sidebar — pick whichever fits your app:

- **`linkComponent`** renders your router's own link component, so you get its prefetching, `aria-current` handling and real `href`s for free. Prefer it when you have one.
- **`navigate`** keeps the plain `<a>` markup and intercepts clicks, calling your callback with the item's `href`. Useful when you have no link component (a custom router, an embedded app, or navigation that runs through your own logic — analytics, unsaved-changes guards).

Passing both is safe: the link component handles the click first, and the sidebar stands down rather than navigating twice.

The interception is deliberately conservative — the browser keeps handling anything that isn't a plain in-app navigation:

| Click | Handled by |
| --- | --- |
| Plain left click on an in-app `href` | `navigate` |
| Ctrl / ⌘ / Shift / Alt click, middle click | Browser (new tab / window) |
| `target: '_blank'` | Browser |
| Absolute URL (`https://…`), protocol-relative (`//…`), `mailto:` | Browser |
| Placeholder / hash-only `href` (`'#'`, `'#section'`) | Browser |
...
| `attrs` | `Record<string, string \| number \| boolean>` | Extra attributes spread onto the rendered link — `data-*` hooks for tests/analytics, `aria-*`, `rel`, `title`. The resolved link target always wins, so `attrs` cannot overwrite `href` / `to`. |
...
| `attrs` | `Record<string, string \| number \| boolean>` | Extra attributes spread onto the group's toggle `<button>`. |
